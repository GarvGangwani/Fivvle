...
import { Coins, FileText, LayoutTemplate, Search, X } from "lucide-react";
import {
  VALIDATION_PAYWALL_CREDITS,
  VALIDATION_PAYWALL_INCLUDES,
...
interface ValidationPaywallModalProps {
  open: boolean;
  onClose: () => void;
  onConfirm: () => void;
  confirming?: boolean;
...
/**
 * Phase 5 — validation paywall; billing runs on POST /experiments/{id}/confirm.
 */
export function ValidationPaywallModal({
  open,
  onClose,
  onConfirm,
  confirming = false,
}: ValidationPaywallModalProps) {
  const [mounted, setMounted] = useState(false);
  const { credits } = useWallet();
  const hasEnoughCredits = credits >= VALIDATION_PAYWALL_CREDITS;
  ...
  return createPortal(
    <div className="fixed inset-0 z-[310] flex items-center justify-center p-4">
      <button
        ...
        aria-label="Close validation paywall"
        onClick={() => {
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="validation-paywall-title"
      ...
      >
        ...
        <div className="px-5 py-5">
          ...
          <ul className="mb-5 space-y-2">
            {VALIDATION_PAYWALL_INCLUDES.map((item) => {
          ...
          {!hasEnoughCredits ? (
            <p className="mb-4 text-xs leading-relaxed text-[var(--fv-danger-light)]">
              You need{" "}
              {(VALIDATION_PAYWALL_CREDITS - credits).toLocaleString()} more
          ...
          <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            ...
            >
              {confirming
                ? "Starting…"
                : `Validate — ${VALIDATION_PAYWALL_CREDITS} Credits`}
            </button>
          </div>
        </div>
      </div>
