...
/**
 * Phase 7 — insight unlock paywall UI only (no billing).
 */
export function InsightPaywallModal({
  open,
  onClose,
  onConfirm,
  confirming = false,
  ...
  const hasEnoughCredits = credits >= INSIGHT_PAYWALL_CREDITS;
  ...
  return createPortal(
    <div className="fixed inset-0 z-[310] flex items-center justify-center p-4">
      <button
        ...
        aria-label="Close insight paywall"
        onClick={() => {
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="insight-paywall-title"
      ...
      >
        <header className="flex items-center justify-between gap-3 border-b border-[var(--fv-border)] px-5 py-4">
          <div>
            <h2
              id="insight-paywall-title"
              className="text-base font-semibold text-[var(--fv-text)]"
            >
              Unlock insight report
            </h2>
            <p className="mt-0.5 text-xs text-[var(--fv-text-muted)]">
              Combine research and behavioral signals into a decision
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="px-5 py-5">
          ...
          <ul className="mb-5 space-y-2">
            {INSIGHT_PAYWALL_INCLUDES.map((item) => {
          ...
          {!hasEnoughCredits ? (
            <p className="mb-4 text-xs leading-relaxed text-[var(--fv-danger-light)]">
              You need {(INSIGHT_PAYWALL_CREDITS - credits).toLocaleString()}{" "}
              more credits. Open your wallet from the header to buy more.
            </p>
          ) : null}
          ...
          <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            ...
            >
              {confirming
                ? "Unlocking…"
                : `Unlock — ${INSIGHT_PAYWALL_CREDITS} Credits`}
            </button>
          </div>
        </div>
      </div>
